...
def timecompare(dir1, bucket):
	...
	if dt2 == -1:
		...
		dt2 = change_time(dt2) #optionally. Please test test your time in testtime.py
		if dt2 != -1:
			...
			try:
				...
				print(dt1.tm_hour,dt1.tm_min,dt1.tm_sec,"!",dt2.tm_hour,dt2.tm_min,dt2.tm_sec)
				return (dt1 > dt2)
			except Exception:
				return True
		else:
...
def serv_loc_run(serv_bucket,other_folders):
	tmp = all_objects(serv_bucket)
	all_files_from_serv = []
	for files in tmp:
		all_files_from_serv.append(files.object_name)
...
def action(bucket):
	directory, time_loc, other_folders = gettime_with_folders()
	if timecompare(directory, bucket) == True:
		run()
		print('loc-web')
	else:
		other_folders.append(directory)
		print("!",other_folders)
		res=minioClient.bucket_exists(args.bucket)
		if res == False:
			make_bucket(args.bucket)			
		serv_loc_run(bucket,other_folders)
		print('web-loc')
